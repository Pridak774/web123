...
document.addEventListener("DOMContentLoaded", function () {
  ...
  updateDateTimeElements(loadingOverlay);
  ...
  if (hamburgerMenu && navMenu) {
    ...
    // Remove all previous event listeners by replacing the element (safe for this context)
    ...
    newHamburger.addEventListener("touchend", toggleMenu);
    ...
    document.addEventListener("click", function (e) {
      if (navMenu.classList.contains("active")) {
        const isClickInside =
          navMenu.contains(e.target) || newHamburger.contains(e.target);
        if (!isClickInside) {
          navMenu.classList.remove("active");
          newHamburger.classList.remove("active");
        }
  ...
  // === DEBUG: Remove overlays and reset pointer-events on mobile ===
  if (window.innerWidth < 768) {
    // Remove loading overlay if present
    var overlay = document.getElementById("loadingOverlay");
    if (overlay) overlay.style.display = "none";
    // Reset pointer-events on body and main containers
    ...
    debug.textContent = "[DEBUG] Overlays removed & pointer-events reset";
...
// Ensure navigation works on DOMContentLoaded
